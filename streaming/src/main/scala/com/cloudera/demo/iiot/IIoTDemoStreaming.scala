...
object IIoTDemoStreaming {
  ...
  def main(args:Array[String]):Unit = {
    ...
    //
    // Hint: Check out the example in for Creating a Direct Stream in Spark Streaming + Kafka Integration guide (https://spark.apache.org/docs/2.2.0/streaming-kafka-0-10-integration.html)
    //       Note that the example assumes Kafka message values are type String; but our Kura/ESF-generated protobuf is binary:
    //       Therefore, ByteArrayDeserializer is the relevant class for "value.deserializer", and we must specify Array[Byte] as the Kafka message value type.
    //
    ...
    */
    ...
    // Parse raw message values into deserialized protobuf objects
    val kurapayloadDStream = kafkaDStream.map(message => {
      val key = message.key

      val value:KuraPayload = {
        // Note: Sometimes Kura/ESF-generated data is GZipped; sometimes it's not. We need to handle both cases.
        val tryUnzipAndParse = Try { KuraPayload.parseFrom(new GZIPInputStream(new ByteArrayInputStream(message.value))) }
        if (tryUnzipAndParse.isSuccess) {
          tryUnzipAndParse.get
        }
        else {
          KuraPayload.parseFrom(message.value)
        }
      }
    ...
    // Note: We define the Scala Case Class "Telemetry" above, which will allow us to use reflection to convert an RDD of "Telemetry" objects into a DataFrame automatically.
    val telemetryDStream = kurapayloadDStream.flatMap(message => {
      ...
      for (metric <- metricsList) {
        val metricValue = {
          // Convert all metrics to String...lazy approach :(
    ...
    //       as well as Spark's documentation for Output Operations on DStreams (https://spark.apache.org/docs/latest/streaming-programming-guide.html#output-operations-on-dstreams).
